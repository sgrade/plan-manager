...
def test_get_story_with_explicit_id():
    ...
    assert result.title == f"Story 2 {suffix}", f"Expected 'Story 2 {suffix}', got {result.title}"
    assert result.description == "Second story", f"Expected 'Second story', got {result.description}"
    ...
    assert result_current.id == story1_id, f"Expected current story_id {story1_id}, got {result_current.id}"
    assert result_current.title == f"Story 1 {suffix}", f"Expected 'Story 1 {suffix}', got {result_current.title}"

    # Test 3: Get story1 with explicit ID - should return story1
    result_story1 = get_story(story_id=story1_id)

    assert result_story1.id == story1_id, f"Expected story_id {story1_id}, got {result_story1.id}"
    assert result_story1.title == f"Story 1 {suffix}", f"Expected 'Story 1 {suffix}', got {result_story1.title}"
    assert result_story1.description == "First story", f"Expected 'First story', got {result_story1.description}"
...
def test_get_story_parameter_types():
    ...
    assert story_id_param.annotation.__origin__.__name__ == 'Union', "story_id should be Optional (Union with None)"

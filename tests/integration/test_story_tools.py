...
def test_get_story_with_explicit_id():
    ...
    set_current_story_id(story1_id, plan_id)

    # Test 1: Get story with explicit ID (story2) - should return story2, not current story
    result = get_story(plan_id=plan_id, story_id=story2_id)
    ...
    result_current = get_story(plan_id=plan_id)
    ...
    result_story1 = get_story(plan_id=plan_id, story_id=story1_id)
...
def test_get_story_with_null_id_requires_current():
    ...
    # Clear current story
    plan = plan_service.create_plan("No Current Story Plan", "test", None)
    plan_id = plan["id"]
    set_current_story_id(None, plan_id)

    # Attempt to get story without ID and without current story should raise ValueError
    with pytest.raises(ValueError, match="Missing required parameter 'story_id'"):
        get_story(plan_id=plan_id)
...
def test_get_story_parameter_types():
    ...
    result = get_story(plan_id=plan_id, story_id=story_id)
    ...
    result_none = get_story(plan_id=plan_id, story_id=None)
